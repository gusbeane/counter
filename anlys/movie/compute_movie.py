...
def _runner(path, snap, COM, ptypes=[0, 1, 2, 3, 4, 6], 
            rng=[[-15, 15], [-15, 15]]):
    ...
    if COM == 'PotMin':
        COM = sn.part1.pos.value[np.argmin(sn.part1.pot.value)]
    
    # Get pos and vel of ptypes
    pos, vel = get_pos_vel(sn, [2, 3, 4, 6])
...
def run(path, name, fout, nsnap, nproc, rng, COM_key):
    ...
    if COM_key == 'BoxCenter':
        ...
        COM_list = np.full(nsnap, COM_key)
    else:
        basepath_COM = '../COM/'
