...
for ang in tqdm(ang_list):

    ang_rad = ang * 2.*np.pi

    Sgr_center = np.array([R_Sgr*np.cos(ang_rad), R_Sgr*np.sin(ang_rad), 0.])
    
    Vx = Vr_Sgr * np.cos(ang_rad) - Vphi_Sgr * np.sin(ang_rad)
    Vy = Vr_Sgr * np.sin(ang_rad) + Vphi_Sgr * np.cos(ang_rad)
    
    Sgr_vel = np.array([Vx, Vy, 0.])
    ...
    ics = arepo.ICs('ics-ang'+str(ang)+'.hdf5', npart, masses=masses)
